...
3. [主な機能](#3-主な機能)
4. [画面構成](#4-画面構成)
5. [データモデル（現行プロトタイプ）](#5-データモデル現行プロトタイプ)
6. [デザイン仕様](#6-デザイン仕様)
7. [技術構成](#7-技術構成)
8. [Swift 側の構成案](#8-swift-側の構成案)
9. [Cloudflare バックエンド構成案](#9-cloudflare-バックエンド構成案)
10. [DB スキーマ案（D1 / SQL）](#10-db-スキーマ案d1--sql)
11. [API エンドポイント案](#11-api-エンドポイント案)
12. [ロードマップ](#12-ロードマップ)
13. [注意点・課題](#13-注意点課題)
...
現時点の成果物は **動作するプロトタイプ（デモ）** です。

| 項目 | 内容 |
| --- | --- |
| 形態 | React 単一ファイル（`AllCarApp.jsx`） |
| データ | すべてモックデータ（アプリ内に埋め込み） |
| 状態管理 | React `useState`（インメモリ、リロードで初期化） |
| 目的 | UI・画面遷移・操作感の検証 |

> プロトタイプは「見た目と操作感」を確認するもので、本番の DB・API・認証・リアルタイム通信は未実装です。本番化の構成は本 README の 7〜11 章にまとめています。
...
- **さがす画面**：参考デザインに合わせ、ダーク背景上に白のブランドカード
...
### 現行（プロトタイプ）
- React（単一ファイル、Tailwind の一部 ＋ 埋め込み CSS）
- アイコン：lucide-react
- 状態：`useState`（インメモリ）
...
| 用途 | 採用 |
| --- | --- |
| UI | **SwiftUI**（宣言的 UI。プロトタイプの構造と相性が良い） |
...
> プロトタイプの各画面（Home / Search / Favorites / Talk / CarDetail）が、そのまま SwiftUI の `Features/` に対応します。SwiftUI も React と同じ「状態から UI を組み立てる」宣言的スタイルなので、プロトタイプのロジックはほぼそのまま移植できます。


---

## 9. Cloudflare バックエンド構成案
...
- **ブランドロゴの商標**：現在は頭文字バッジで代用。実ロゴを使う場合は各メーカーの利用許諾が必要。
- **リアルタイムトーク**：アカウント・友達関係・メッセージ保存・通知が必要。Cloudflare では Durable Objects で実現できるが、設計と課金の確認を。
- **車種とグレード**：現状は「1 車種＝1 レコード」。グレード（AT/MT 違いなど）を扱うなら、車種の下にグレードをネストする構造に拡張する。
- **プロトタイプの永続化**：現行はインメモリのため、リロードで初期化される。本番では上記 API に接続して永続化する。

---

### 成果物
- `AllCarApp.jsx` … 現行プロトタイプ（React）
- `README.md` … 本ドキュメント